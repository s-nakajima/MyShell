 ...
 * Controllerのテストファイル生成クラス
 */
Class CreateController4AddController extends CreateController4 {
	...
	public function createTest($param, $action = null) {
		...
		if ($this->isManagerPlugin()) {
			...
			$process = array(
				...
				'//テスト実行',
				'$this->_testGetAction(',
				chr(9) . 'array(\'action\' => \'add\', \'block_id\' => $blockId, \'frame_id\' => $frameId),',
				chr(9) . 'array(\'method\' => \'assertNotEmpty\'), null, \'view\'',
				');',
				'',
				'//チェック',
				'$this->__assertAddGet($frameId, $blockId, $blockKey);',
			);
		}
		$output .=
			$this->_classMethod(
		...
		if ($this->isManagerPlugin()) {
			$output .=
				$this->_classMethod(
					...
					array(
						...
						'',
						'$this->assertInput(\'form\', null, \'/' . Inflector::underscore($this->plugin) . '/' . $controller . '/add\', $result);',
			...
			$output .=
				$this->_classMethod(
					...
					'__assertAddGet($frameId, $blockId, $blockKey)',
					array(
						'$result = $this->_parseView($this->view);',
						'',
						'//TODO:必要に応じてassert書く',
						...
						'',
						'$this->assertInput(\'form\', null, \'' . Inflector::underscore($this->plugin) . '/' . $controller . '/add/\' . $blockId, $result);',
						'$this->assertInput(\'input\', \'_method\', \'POST\', $result);',
